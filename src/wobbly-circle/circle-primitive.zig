...
pub fn createCirclePrimitive(allocator: std.mem.Allocator, vertices: u32, radius: f32, time: f32) !Primitive {
    ...
    const phase: f32 = @sqrt(@mod(time, 100)) * 8 * PI;

    const magniture: f32 = radius * (zmath.cos(time) + 1) / 20;

    while (x < vertices) : (x += 1) {
        const x_f = @as(f32, @floatFromInt(x));

        const current_angle = angle * x_f;
        const current: f32 = zmath.cos(frequency * current_angle + phase);

        const modified_radius = radius + magniture * current;
        const pos_x = modified_radius * zmath.cos(angle * x_f);
        const pos_y = modified_radius * zmath.sin(angle * x_f);
